...
class HREmployeeLoanType(models.Model):
    ...
    def _compute_loan_done(self):
        for record in self:
            emp_loan = self.env['hr.employee.loan'].search([('state','=','done'),('loan_type_id','=',record.id)])
            record.count_loan_done = len(emp_loan)
            
    def _compute_loan_paid(self):
        for record in self:
            emp_loan = self.env['hr.employee.loan'].search([('state','=','paid'),('loan_type_id','=',record.id)])
            record.count_loan_paid = len(emp_loan)
            
    def _compute_loan_draft(self):
        for record in self:
            emp_loan = self.env['hr.employee.loan'].search([('state','=','draft'),('loan_type_id','=',record.id)])
