...
class StockInventoryLine(models.Model):
    ...
    prod_lot_id = fields.Many2one(
        'stock.lot', 'Número de Lote',
        domain="[('product_id','=',product_id), ]", )
    package_id = fields.Many2one(
        'stock.quant.package', 'Paquete', index=True)
    partner_id = fields.Many2one('res.partner', 'Responsable',)
    available_quantity = fields.Float(
        'Cantidad Disponible    ',
        help="Cantidad a mano que no ha sido reservada para una Orden de Entrega en su Unidad de Medida por Defecto",
        digits='Product Unit of Measure', readonly=True)
    quantity = fields.Float(
        'Cantidad a Mano',
        help='Cantidad del Producto a Mano en su Unidad de Medida por Defecto',
        readonly=True, 
        digits='Product Unit of Measure')
    inventory_quantity = fields.Float(
        'Cantidad Contada', digits='Product Unit of Measure',
        help="Cantidad Contadad del Producto.")
    make_it_zero = fields.Boolean('Colocar en Cero',)
    state = fields.Selection(
        string='Estado',  related='stock_inventory_id.state', readonly=True)
    company_id = fields.Many2one(
        'res.company', 'Compañía', related='stock_inventory_id.company_id',
        index=True, readonly=True, store=True)

    lot_serial = fields.Boolean(string='Serial', compute="compute_lot_serial_require")
    lot_id = fields.Boolean(string='Lote', compute="compute_lot_serial_require")
    currency_id = fields.Many2one('res.currency', 'Moneda', related='company_id.currency_id', readonly=True, required=True)
    price_value = fields.Monetary('Diferencia en Valoración', compute="compute_total_price")
    reserved_quantity = fields.Float('Cantidad Reservada',readonly=True, digits='Product Reserved Quantity', compute="_compute_reserved_quantity", store=True)
