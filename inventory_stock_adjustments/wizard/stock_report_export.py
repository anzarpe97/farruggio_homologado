...
class InventoryStockExport(models.TransientModel):
    ...
    start_date = fields.Date('Fecha de inicio', default=lambda self: fields.Date.today())
    end_date = fields.Date('Fecha de fin', default=lambda self: fields.Date.today())
    ...
    def action_report_excel(self):
        ...
        if self.start_date and self.end_date:
            ...
            raise UserError(_("No has seleccionado la fecha de inicio y de finalización. Por favor, verifica que las fechas estén correctamente establecidas."))
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write('A4', 'Desde Fecha: ', bold_format)
        sheet.write('A5', 'Hasta Fecha: ', bold_format)
        sheet.merge_range('C1:G2', 'Reporte de Movimiento de Inventario', head)
        headers = ['Ubicación', 'Producto', 'UdM', 'N° de Lote','Dif. Valoración', 'Cantidad Disponible','Cantidad Reservada','Cantidad en Mano','Cantidad Contada']
