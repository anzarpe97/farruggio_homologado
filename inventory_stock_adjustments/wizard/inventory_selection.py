...
class InventorySelection(models.TransientModel):
    ...
    add_product_qty = fields.Integer(string="Añadir Cantidad")
...
class InventorySelectionLine(models.TransientModel):
    ...
    lot_id = fields.Boolean(string='Lote',readonly=True, compute="compute_lot_serial_required")
    lot_serial = fields.Boolean(string='Serial',readonly=True, compute="compute_lot_serial_required")
    quantity_on_hand = fields.Float(
        'Cantidad a Mano',
        readonly=True, 
        digits='Product Unit of Measure')
    quantity = fields.Float(string="Cantidad", default=get_default_qty)
    prod_lot_id = fields.Many2one(
        'stock.lot', 'Número de Lote')
    stock_inventory_line_id = fields.Many2one('stock.inventory.line')
    product_id = fields.Many2one('product.product', related="inventory_selection_id.product_id", string="Producto")
