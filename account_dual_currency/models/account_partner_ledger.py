            ...
            if currency_dif == self.env.company.currency_id.symbol:
                queries.append(f"""
                    SELECT
                        ...
                        SUM(ROUND(account_move_line.debit, currency_table.precision))   AS debit,
                        SUM(ROUND(account_move_line.credit, currency_table.precision))  AS credit,
                        SUM(ROUND(account_move_line.balance, currency_table.precision)) AS balance
                    FROM {tables}
            ...
            if currency_dif == self.env.company.currency_id.symbol:
                queries.append(f'''
                    SELECT
                        ...
                        ROUND(account_move_line.debit, currency_table.precision)   AS debit,
                        ROUND(account_move_line.credit, currency_table.precision)  AS credit,
                        ROUND(account_move_line.balance, currency_table.precision) AS balance,
            ...
            # -- DOLARES --
                ...
                queries.append(f'''
                    SELECT
                        ...
                        0 as amount_currency,
                        account_move_line.matching_number,
                        CASE WHEN aml_with_partner.balance > 0 THEN 0 ELSE partial.amount_usd END               AS debit,
                        CASE WHEN aml_with_partner.balance < 0 THEN 0 ELSE partial.amount_usd END               AS credit,
                        CASE WHEN aml_with_partner.balance > 0 THEN -partial.amount_usd ELSE partial.amount_usd END AS balance,
                        account_move.name                                                                   AS move_name,
                        account_move.move_type                                                              AS move_type,
                        account.code                                                                        AS account_code,
                        {account_name}                                                                      AS account_name,
