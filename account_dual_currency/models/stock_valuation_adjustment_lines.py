...
class AdjustmentLines(models.Model):
    ...
    # Campos informativos en USD (no utilizados en los asientos contables)
    ...
    final_cost_usd = fields.Monetary(currency_field='currency_id_dif', string='Nuevo Valor $', compute='_compute_final_cost_usd', store=True)
    ...
    def _create_account_move_line(self, move, credit_account_id, debit_account_id, qty_out, already_out_account_id):
        ...
        Los cálculos contables se hacen únicamente con la moneda local.
        Los campos en USD no afectan estos asientos, son informativos.
        """
        AccountMoveLine = []
        base_line = {
            'name': self.name,
        ...
        if qty_out > 0:
            debit_line = dict(base_line,
                              name=f"{self.name}: {qty_out} {_('already out')}",
                              quantity=0,
