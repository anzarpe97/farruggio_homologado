...
class AccountBankStatementLine(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            # 1. Asegúrate que la fecha esté presente
            fecha = vals.get('date')
            currency_id_dif = vals.get('currency_id_dif_statement') \
                or self.env.company.currency_id_dif.id

            tasa = 0
            if fecha and currency_id_dif:
                # Busca tasa por fecha, moneda y compañía
                domain = [
                    ('name', '=', fecha),
                    ('currency_id', '=', currency_id_dif),
                    ('company_id', '=', self.env.company.id)
                ]
                tasa_obj = self.env['res.currency.rate'].search(domain, limit=1)
                tasa = tasa_obj.inverse_company_rate if tasa_obj else self.env.company.currency_id_dif.inverse_rate
            else:
                tasa = self.env.company.currency_id_dif.inverse_rate

            # 2. Aplica la tasa SÓLO si no viene en el archivo/importación
            if not vals.get('tasa_referencia_statement'):
