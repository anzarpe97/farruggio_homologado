				...
				if(pos_config.restrict_zero_qty){
					$.each(lines, function( i, line ){
						...
						if (prd.type == 'product'){
							if(prd.qty_available <= 0){
								restrict = true;
								call_super = false;
								let wrning = prd.display_name + ' No tiene existencia suficiente.';
								self.showPopup('ErrorPopup', {
									title: self.env._t('No hay existencia en inventario'),
					...
					if(restrict === false){
						$.each(prod_used_qty, function( i, pq ){
							...
							let check = pq[0] - pq[1];
							let wrning = product.display_name + ' No tiene existencia suficiente.';
