...
class AccountTax(models.Model):
    ...
    def get_withholding_vals(self, payment_group):
        ...
        if self.withholding_type == 'partner_tax' and payment_group.iva == True:
            ...
            amount = (payment_group.to_pay_move_line_ids[0].move_id.amount_tax_bs * (alicuota)  ) #/ payment_group.to_pay_move_line_ids[0].move_id.tax_day
            logging.info(base_amount)
            logging.info(amount)
            ...
            vals['currency_id'] = self.env['res.currency'].search([('name','=','VEF')]).id
            ...
            for band in regimen.banda_calculo_ids:
                ...
                if regimen.type_subtracting == 'amount' and \
                    ...
                    subtractingBS = band.withholding_amount_bs * \
                    regimen.seniat_ut_id.amount
            ...
            if subtracting > 0.0:
                ...
                withholdingBs = (payment_group.to_pay_move_line_ids[0].move_id.amount_untaxed_bs *
                               withholding_percentage) - subtractingBS
            else:
                withholding = base_withholding * withholding_percentage
                withholdingBs = payment_group.to_pay_move_line_ids[0].move_id.amount_untaxed_bs * withholding_percentage


            logging.info(withholding)
            logging.info(withholdingBs)
            ...
            vals['period_withholding_amount'] = withholdingBs
