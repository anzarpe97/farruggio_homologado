...
class accessFieldAccess(models.Model):
    ...
    access_field_readonly = fields.Boolean(string='Solo Lectura')
    access_field_required = fields.Boolean(string='Requerido')
    access_field_external_link = fields.Boolean(string='Remover Link Externo')
    access_user_manager_id = fields.Many2one('user.management', string='Administrar')
    ...
    def access_check_field_access(self):
        for rec in self:
            if (rec.access_field_required and rec.access_field_readonly) or (rec.access_field_required and rec.access_field_invisible):
                raise UserError(_('No puedes seleccionar este campo como Solo Lectura y Requerido al Mismo Tiempo'))
