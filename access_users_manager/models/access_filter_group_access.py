...
class accessFilterGroupAccess(models.Model):
    ...
    access_hide_filter_ids = fields.Many2many('filter.group.data', string='Ocultar Filtros')
    access_hide_group_ids = fields.Many2many('filter.group.data', 'group_access_rel', 'access_id', 'data_id',
                                         string='Ocultar Grupos')
    access_user_manager_id = fields.Many2one('user.management', string='Reglas')
...
class accessFilterGroupData(models.Model):
    ...
    access_model_id = fields.Many2one('ir.model', string='Modelo', index=True, ondelete='cascade', required=True)
    access_model_name = fields.Char(string='Nombre del Modelo', related='access_model_id.model', readonly=True, store=True)
    access_type = fields.Selection([('filter', 'Filter'), ('group', 'Group')], string="Type",
                            required=True)
    access_filter_group_string = fields.Char('Filtro / Nombre del Grupo', required=True)
