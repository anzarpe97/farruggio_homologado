...
class accessResUserProfiles(models.Model):
    ...
    group_id = fields.Many2one(
        ...
        ondelete="cascade",
        readonly=True,
        string="Grupo Asociado",
    )

    access_line_ids = fields.One2many(
        comodel_name="user.profile.lines", inverse_name="access_profile_id", string="Líneas del Perfil"
    )
    access_user_ids = fields.Many2many(
        "res.users", "user_profiles_users_rel", "user_id", "profile_id", string="Usuarios"
    )
    access_record_rule_ids = fields.Many2many(
        comodel_name="ir.rule",
        compute="_compute_rule_ids",
        string="Reglas de Registro",
        required=False,
    )
