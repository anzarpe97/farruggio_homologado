...
class ResGroups(models.Model):
    ...
    access_profile_id = fields.One2many(
        ...
        help="Relación  para el grupo que representa a los perfiles",
    )
    ...
    trans_parent_ids = fields.Many2many(
        ...
        string="Grupo Principal",
        compute="_compute_trans_parent_ids",
        recursive=True,
    )

    access_profiles_count = fields.Integer("# Perfiles", compute="_compute_profiles_count")
        ...
        if self._context.get('install_filename') or self._context.get(MODULE_UNINSTALL_FLAG):
            ...
            xml = E.field(
                *(xml0),
                ...
                position="replace")
            xml.addprevious(etree.Comment("Generado automáticamente por grupos"))
