...
class accessResUsersInherit(models.Model):
    ...
    access_password_update = fields.Datetime('Última Actualización de Contraseña', default=datetime.now())
    access_password_expire_date = fields.Date(string='Fecha de Contraseña Vencida')
    access_recent_activity_line = fields.One2many('recent.activity', 'access_user_id', string='Actividad Reciente', readonly=True,
                                              _compute='compute_duration')
    access_admin_user = fields.Boolean(compute='compute_is_admin_user', string='Usuario Administrador', store=True)
