...
const PosDisableAddProduct = (ProductScreen) =>
    class extends ProductScreen {
        _clickProduct(product) {
            if (this.env.pos.config.allow_pos_add_product_button) {
                super._clickProduct(product);
            }
        }
        _setValue(val) {
            if (this.currentOrder.get_selected_orderline()) {
                if (this.env.pos.numpadMode === 'quantity' && this.env.pos.config.allow_pos_qty_button) {
                    const result = this.currentOrder.get_selected_orderline().set_quantity(val);
                    if (!result) NumberBuffer.reset();
                } else if (this.env.pos.numpadMode === 'discount' && this.env.pos.config.allow_pos_discount_button) {
                    this.currentOrder.get_selected_orderline().set_discount(val);
                } else if (this.env.pos.numpadMode === 'price' && this.env.pos.config.allow_pos_price_button) {
...
const PosDisableOrderEditNumpadWidget = (NumpadWidget) =>
    class extends NumpadWidget {
        ...
        getButtons() {
            return [
                { selector: ".mode-button:nth-child(4)", configField: "allow_pos_qty_button", position: 4 },
                { selector: ".mode-button:nth-child(8)", configField: "allow_pos_discount_button", position: 8 },
                { selector: ".mode-button:nth-child(12)", configField: "allow_pos_price_button", position: 12 },
                { selector: ".input-button.numpad-backspace", configField: "allow_pos_delete_button", position: 0 },
            ];
        }
...
const PosDisableOrderEditActionWidget = (ActionpadWidget) =>
    class extends ActionpadWidget {
        ...
        _onMounted() {
            const buttons = [
                { selector: ".set-partner", configField: "allow_pos_customer_button" },
                { selector: ".pay", configField: "allow_pos_payment_button" },
            ];
