odoo.define("pos_feature_in_one.PosScreenLock", function (require) {
    "use strict";
    ...
    const PosAutoLockPoModel = (PosGlobalState) => class PosAutoLockPoModel extends PosGlobalState {
        constructor(obj) {
            super(obj);
    ...
    Registries.Model.extend(PosGlobalState, PosAutoLockPoModel);
    ...
    const PosResChrome = (Chrome) =>
        class extends Chrome {
            ...
            async start() {
                    await super.start()
                    var self = this;
                    if (this.env.pos.config.pos_auto_lock) {
                        var set_logout_interval = function (time) {
                            time = time || self.env.pos.config.pos_lock_timer * 1000;
                            if (time) {
                    ...
                    if (this.env.pos.config.pos_auto_lock && this.env.pos.config.pos_lock_timer) {
                        $(document).on("click", async function (event) {
                            if (self.env.pos.config.pos_auto_lock && self.env.pos.config.pos_lock_timer) {
