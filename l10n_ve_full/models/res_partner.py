...
class ResPartner(models.Model):
    ...
    wh_iva_purchase_rate = fields.Float(
    ...
    sica = fields.Char(string='Código SICA', help="Código del Sistema Integral de Control Agroalimentario")

    def _display_address(self, without_company=False):
        address_format = super(ResPartner, self)._display_address(without_company=without_company)
        if self.sica:
            address_format += f"\nSICA: {self.sica}"
    ...
    def _address_fields(self):
        ...
        address_fields.append('sica')  # Añade SICA a los campos de dirección
    ...
    islr_withholding_agent = fields.Boolean(
        '¿Agente de retención de ISLR?', default=True,
        help="Verifique si el partner es un agente de retención de ISLR")
    spn = fields.Boolean(
    ...
    islr_exempt = fields.Boolean(
        '¿Está exento de retención de ISLR?',
        help='Si el individuo está exento de retención de ISLR')
    ...
    economy_activiy = fields.Char(string="Actividad Económica", help="Campo para añadir la Actividad Económica.")
        ...
        for vals in vals_list:
            if vals.get('vat'):
                if vals.get('company_type') == 'person':
                    vals['identification_id'] = vals['vat']
                else:
                    vals['rif'] = vals['vat']
            else:
                vals['identification_id'] = vals.get('vat')

            if vals.get('identification_id'):
                if vals.get('nationality'):
                    self.validation_document_ident(vals['identification_id'], vals['nationality'])
                ...
                if existing:
                    raise UserError(_('Ya existe un contacto con este documento de identidad: %s') % vals['identification_id'])

            if vals.get('rif'):
                if self.env.company.country_id.code == 'VE':
                    if not self.validate_rif_er(vals['rif']):
                        raise UserError(_('El RIF tiene el formato incorrecto. Ej: V-01234567-8'))
                    ...
                    if existing:
                        raise UserError(_('Ya existe un contacto con este RIF: %s') % vals['rif'])

            if vals.get('email') and not self.validate_email_addrs(vals['email'], 'email'):
                raise UserError(_('El email es incorrecto. Ej: cuenta@dominio.xxx'))

        # Crear los contactos
        partners = super().create(vals_list)

        # Post-procesamiento por contacto
        for partner, vals in zip(partners, vals_list):
            if vals.get('rif'):
                partner.vat = vals['rif']
            if partner.sica and partner.child_ids:
                partner.child_ids.write({'sica': partner.sica})
    ...
    def write(self, vals):
        ...
        if vals.get('identification_id'):
            nationality = vals.get('nationality', self.nationality)
            if nationality:
                self.validation_document_ident(vals['identification_id'], nationality)

        if vals.get('rif') and self.country_id and self.country_id.code == 'VE':
            if not self.validate_rif_er(vals['rif']):
                raise UserError(_('El RIF tiene el formato incorrecto. Ej: V-01234567-8'))

        if vals.get('rif'):
            vals['vat'] = vals['rif'].upper()

        if vals.get('email'):
            if not self.validate_email_addrs(vals['email'], 'email'):
                raise UserError(_('El email es incorrecto. Ej: cuenta@dominio.xxx'))

        res = super().write(vals)

        if 'sica' in vals and self.child_ids:
            self.child_ids.write({'sica': vals['sica']})
    ...
    def _check_unique_identification_id(self):
        for partner in self:
            if partner.identification_id and partner.company_type == 'person':
                existing = self.search([
                    ('identification_id', '=', partner.identification_id),
                    ('id', '!=', partner.id)
                ], limit=1)
                if existing:
                    raise UserError('Ya existe un contacto con este documento de identidad: %s' % partner.identification_id)

    @api.constrains('rif')
    def _check_unique_rif(self):
        for partner in self:
            if partner.rif and partner.company_type == 'company':
                existing = self.search([
                    ('rif', '=', partner.rif),
                    ('id', '!=', partner.id)
                ], limit=1)
                if existing:
                    raise UserError('Ya existe un contacto con este RIF: %s' % partner.rif)
