...
class AccountWhIvaLineTax(models.Model):
    ...
    def _get_amount_ret(self):
        """ Calcula el monto de la retención basado en el monto y tasa de retención real. """
        for record in self:
            tasa = record.wh_vat_line_id.wh_iva_rate or 0.0
            if record.amount and tasa:
                record.amount_ret = round(record.amount * (tasa / 100.0), 2)
            else:
