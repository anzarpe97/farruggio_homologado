...
class ResCompany(models.Model):
    ...
    wh_islr_sale_journal_id = fields.Many2one(
        'account.journal',
        string="Diario de Retención ISLR (Ventas)",
        domain="[]"
    )
    wh_islr_purchase_journal_id = fields.Many2one(
        'account.journal',
...
class AccountWhIslrDoc(models.Model):
    ...
    def _get_journal(self, partner_id=None):
        tipo = self._context.get('default_type') or self._context.get('type')
        company = self.env.company
        if tipo in ('out_invoice', 'out_refund'):
            return company.wh_islr_sale_journal_id
        elif tipo in ('in_invoice', 'in_refund'):
            return company.wh_islr_purchase_journal_id
        return False
    ...
    def action_done(self):
        ...
        self.write({'state': 'done', 'confirming_user_id': self.env.user.id})
    ...
    def action_confirm(self):
        ...
        """
        for doc in self:
            if doc.amount_total_ret < 1:
                raise UserError(_("No es posible generar el comprobante de retención de ISLR debido a que el monto a retener es menor a 1. Por favor, verifique los cálculos o ajuste los montos para continuar."))
    ...
    def action_move_create(self):
        ...
        for line in ret.invoice_ids:
            ...
            if ret_move and ret_move.exists():
                tax_to_copy = None
                ...
                if invoice.tax_today and invoice.tax_today != 1.0:
                    ...
                    # Asume que tienes disponible el helper _get_tasa_usd_by_date en account.move
                    tax_to_copy = invoice._get_tasa_usd_by_date(fecha, invoice.company_id)
                if tax_to_copy and tax_to_copy != 1.0:
                    ret_move.with_context(skip_tax_today_update=True).write({'tax_today': tax_to_copy})
            # ------------------------------------------------------------------
    ...
    def print_wh_islr_report(self):
        self.ensure_one()
        return self.env.ref('l10n_ve_full.action_withholdong_islr_report').report_action(self)
