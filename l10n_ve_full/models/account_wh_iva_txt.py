...
class TxtIva(models.Model):
    ...
    def get_document_affected(self, txt_line):
        """
        Retorna el número del documento afectado dependiendo del tipo de devolución:
        
        - Para devoluciones de clientes (out_refund): extrae el número que aparece después de la cadena
        "Reversión de:" en el campo 'name' de la factura.
        - Para devoluciones de proveedores (in_refund): retorna el valor del campo 'invoice_reverse_purchase_id.name'.
        
        Si no se cumple ninguna condición, retorna '0'.
        
        @param txt_line: línea del documento actual.
        """
        if not txt_line.invoice_id:
            return '0'
        
        invoice = txt_line.invoice_id
        if invoice.move_type == 'out_refund':
            # Extraer el número que aparece después de "Reversión de:" en el nombre de la factura.
            name = invoice.name or ""
            if "Reversión de:" in name:
                parts = name.split("Reversión de:")
                if len(parts) > 1:
                    return parts[1].strip() or '0'
            return '0'
        elif invoice.move_type == 'in_refund':
            # Para devolución de proveedores, obtener el nombre del documento afectado.
            if invoice.invoice_reverse_purchase_id:
                return invoice.invoice_reverse_purchase_id.supplier_invoice_number or '0'
            return '0'
        else:
            return '0'
    ...
    def generate_txt(self):
        ...
        for txt in self:
            ...
            for txt_line in txt.txt_ids:
                ...
                control_number = self.get_number(txt_line.invoice_id.correlative, 'inv_ctrl', 20)
    ...
    def formato_cifras(monto):
        try:
            return '{:.2f}'.format(float(monto))  # convierte a float, redondea y limita a 2 decimales
