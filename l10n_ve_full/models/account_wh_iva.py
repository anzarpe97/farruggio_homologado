...
class ResCompany(models.Model):
    ...
    wh_iva_sale_journal_id = fields.Many2one(
        'account.journal', string="Diario de Retención de IVA (Ventas)",
        domain="[]"
    )
    wh_iva_purchase_journal_id = fields.Many2one(
        'account.journal', string="Diario de Retención de IVA (Compras)",
        domain="[]"
    )
    wh_iva_sale_rate = fields.Float(
        string="Alícuota Retención IVA (Ventas)",
        help="Tasa de retención de IVA para facturas de clientes"
    )
...
class AccountWhIva(models.Model):
    ...
    def get_wh_iva_rate(self):
        """Devuelve la tasa de retención de IVA correcta según el tipo de documento."""
        self.ensure_one()
        if self.type in ['out_invoice', 'out_refund', 'out_debit']:
            # Retención sobre ventas: la tasa está en la compañía
            return self.company_id.wh_iva_sale_rate or 0.0
        elif self.type in ['in_invoice', 'in_refund', 'in_debit']:
            # Retención sobre compras: la tasa está en el proveedor
            return self.partner_id.wh_iva_purchase_rate or 0.0
        return 0.0
    ...
    def create(self, values):
        ...
        # Defensa: si por algún flujo externo se crea directamente en 'done'
        # sin pasar por confirm_check(), registrar el usuario creador como confirmador.
        if (
            values.get('state') == 'done'
            and not values.get('confirming_user_id')
            and not wh_iva_id.confirming_user_id
        ):
            wh_iva_id.confirming_user_id = self.env.user
        return wh_iva_id

    def write(self, vals):
        res = super(AccountWhIva, self).write(vals)

        # Defensa: algunos flujos pueden cambiar a 'done' sin setear confirming_user_id.
        # En ese caso, asignamos el usuario que ejecuta el write.
        if (
            vals.get('state') == 'done'
            and not vals.get('confirming_user_id')
        ):
            missing = self.filtered(lambda r: r.state == 'done' and not r.confirming_user_id)
            if missing:
                # No levantar error: solo completar el dato faltante.
                super(AccountWhIva, missing).write({'confirming_user_id': self.env.user.id})
        return res
    ...
    def _get_journal(self):
        type = self._context.get('default_type') or self._context.get('type')
        company = self.env.company  # O self.env.user.company_id
        if type in ('out_invoice', 'out_refund', 'out_debit'):
            return company.wh_iva_sale_journal_id
        elif type in ('in_invoice', 'in_refund', 'in_debit'):
            return company.wh_iva_purchase_journal_id
        return False
    ...
    # En el modelo AccountWhIva, el campo date_ret parece ser la fecha de retención:
    @api.model
    ...
    def check_invoice_nro_ctrl(self):
        ...
        for obj in self:
            for wh_line in obj.wh_lines:
                if not wh_line.invoice_id.correlative:
                    res[wh_line.id] = (
                        wh_line.invoice_id.name,
                     #   wh_line.invoice_id.number,
                        wh_line.invoice_id.supplier_invoice_number)
        ...
        for ret in self.with_context(ctx):
            ...
            if ret.wh_lines:
                ...
                for line in ret.wh_lines:
                    ...
                    # --- Nueva lógica: forzar que el move de retención use la misma tasa que la factura ---
                    if invoice.tax_today and invoice.tax_today != 1.0:
                        ret_move.with_context(skip_tax_today_update=True).write({'tax_today': invoice.tax_today})
    ...
    def confirm_check(self):
        ...
        if (not self.check_wh_lines() or
            ...
            # Consolidar todas las operaciones de write en una sola llamada
            write_vals = {
                'date_ret': self.date_ret,
                'state': 'done', 
                'confirming_user_id': self.env.user.id
            }
            if self.type in ('in_invoice', 'in_refund', 'in_debit') and not self.number:
                write_vals['number'] = self.number
    ...
    confirming_user_id = fields.Many2one(
        'res.users', string='Usuario Confirmador',
        readonly=True, copy=False,
        help="Usuario que confirmó esta retención")
    ...
    def _amount_ret_all(self):
        for rec in self:
            ...
            wh_iva_rate = rec.get_wh_iva_rate()
            if rec.create_date and rec.wh_lines:
                rec.total_tax_ret = sum(l.amount_tax_ret for l in rec.wh_lines)
                rec.amount_base_ret = sum(l.base_ret for l in rec.wh_lines)
                rec.withholding_rate = wh_iva_rate  # Esta es la tasa real aplicada (para mostrar en la cabecera)
                # Además, actualiza la tasa en cada línea si hace falta
                for l in rec.wh_lines:
                    l.wh_iva_rate = wh_iva_rate

    def print_wh_iva_report(self):
        self.ensure_one()
        return self.env.ref('l10n_ve_full.action_withholdong_vat_report').report_action(self)
