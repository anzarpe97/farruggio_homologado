...
class RepComprobanteIslr(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['form'].state == 'done':
            factura = data['form'].invoice_ids.invoice_id
            # Compañía: moneda base
            currency_company = data['form'].company_id.currency_id
            # Moneda de la factura
            currency_invoice = factura.currency_id

            if currency_invoice.id == currency_company.id:
                total_doc = factura.amount_total
            else:
                tasa = factura.tax_today
                if tasa:
                    total_doc = factura.amount_total * tasa
                else:
                    tasa = self.obtener_tasa(factura)
                    total_doc = factura.amount_total * tasa
    ...
    def _get_nro_ctrl(self, id):
        ...
        nro_ctrl = id[0].invoice_id.correlative
        return nro_ctrl
